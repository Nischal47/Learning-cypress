export class SwagLab {
    ...
    /******  56ce111b-3d97-46ed-8dd8-3e90ce1f79f6  *******/
    sortProducts(type = "name_asc") {
        switch (type) {
            case "name_asc":
                ...
                break;
            case "name_desc":
                cy.get("select").select("Name (Z to A)").should("have.value", "za");
                cy.get(".inventory_item").should("have.length.gt", 0)
                    .then((products) => {
                        const productNames = products.map((index, item) =>
                            Cypress.$(item).find(".inventory_item_name").text()
                ...
                break;
            case "price_asc":
                cy.get('select').select('Price (low to high)').should('have.value', 'lohi');
                cy.get('.inventory_item').should('have.length.gt', 0)
                    .then((products) => {
                ...
                break;
            case "price_desc":
                cy.get("select").select("Price (high to low)").should("have.value", "hilo");
                cy.get(".inventory_item").should("have.length.gt", 0)
                    .then((products) => {
                        const productPrices = products.map((index, item) =>
                            Cypress.$(item).find(".inventory_item_price").text()
                        )
                        const sortedPrices = [...productPrices].sort().reverse()
                        expect(productPrices.get()).to.deep.equal(sortedPrices)
                    })
                break;
            default:
